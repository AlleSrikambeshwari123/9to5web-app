...
router.post("/req-pwd-reset", function (req, res, next) {
  ...
  services.customerService
    .requestResetPassword(req.body.skybox)
    .then((requestResult) => {
...
router.get("/get-packages/:id", (req, res, next) => {
  services.packageService
    .getCustomerPackages(req.params.id)
    .then((packages) => {
      Promise.all(
        packages.map((pkg) => {
...
router.get("/get-packages-history", function (req, res, next) {
  ...
  res.send({
    packages: [
      {
        packageId: 34,
        trackingNumber: "114-3477688-4697861",
        description: "Fog Lamps",
        dateRec: "2019-07-2",
        dateDelivered: "2019-07-07",
        cost: 250.0,
        status: "Delivered",
        statusId: 5,
      },
      {
        packageId: 35,
        trackingNumber: "114-2440197-3408215",
      ...
      {
        packageId: 36,
        trackingNumber: "114-0354742-6210612",
