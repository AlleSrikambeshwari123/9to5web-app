...
class PaidTypeService {
  ...
  async getPaidTypes(req){
    ...
    var columns = {0:'name', 1: 'createdAt'} 
    ...
    var totalPaidType = await PaidType.count(searchData);
    return new Promise(async(resolve, reject) => {
      PaidType.find(searchData)
      .sort({[sortField]:sort})
      .skip(start)
      .limit(length)
      .exec((err, result) => {
        if (err) {
          ...
          resolve({paidTypes:result, total: totalPaidType});
        }
      })
    })
  }
