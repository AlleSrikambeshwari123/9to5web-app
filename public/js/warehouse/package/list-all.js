...
$(function(){
  ...
  $.fn.dataTable.ext.search.push(
    function (settings, data, dataIndex) {
      var startDate = moment(jQuery(packageTable.table().node()).data('startDate')).format("YYYY/MM/DD");
      var endDate = moment(jQuery(packageTable.table().node()).data('endDate')).format("YYYY/MM/DD");
      if (jQuery(packageTable.table().node()).data('startDate') == null || jQuery(packageTable.table().node()).data('endDate') == null) {
        return true;
      }
      var dateTime = new Date(data[11]).getTime();
  ...
  $(document).ready(function () {
    $('input[name="daterange"]').daterangepicker({
      ...
      $(packageTable.table().node()).data('startDate', start);
      $(packageTable.table().node()).data('endDate', end);
      packageTable.draw();
    });
    ...
    $('#daterange').on('cancel.daterangepicker', function (ev, picker) {
      ...
      $('#daterange').val('');
      $(packageTable.table().node()).data('startDate', null);
      $(packageTable.table().node()).data('endDate', null);
      packageTable.draw();
    });
    $(packageTable.table().node()).data('startDate', moment().startOf('hour').subtract(21, 'days'));
    $(packageTable.table().node()).data('endDate', moment());
    packageTable.draw();
  });
})