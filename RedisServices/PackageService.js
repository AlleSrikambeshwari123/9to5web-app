...
class PackageService {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let result = await Promise.all(
        packages.map(async (item) => {
          let packagesStatus = await PackageStatus.find({
            packageId: item._id,
          });
          ...
          obj["received_fill"] =
            obj["received_fill"] +
            (packagesStatus.status == PKG_STATUS[1] ? 1 : 0);
          obj["loaded_craft"] =
            obj["loaded_craft"] +
            (packagesStatus.status == PKG_STATUS[2] ? 1 : 0);
          obj["in_transit"] =
            obj["in_transit"] +
            (packagesStatus.status == PKG_STATUS[3] ? 1 : 0);
          obj["received_nas"] =
            obj["received_nas"] +
            (packagesStatus.status == PKG_STATUS[4] ? 1 : 0);
          obj["ready_pd"] =
            obj["ready_pd"] + (packagesStatus.status == PKG_STATUS[5] ? 1 : 0);
          obj["delivered"] =
            obj["delivered"] + (packagesStatus.status == PKG_STATUS[6] ? 1 : 0);
      ...
      Package.find(
        ...
        async (err, packages) => {
          if (err) {
            ...
            const packagesStatus = await PackageStatus.find({
            ...
            Object.keys(packageStatusDataById).forEach((packageId) => {
              ...
              const lastStatusData =
                dataByPackageId[dataByPackageId.length - 1];
              const date = moment(lastStatusData.packageId.createdAt).format(
                "DD/MM"
              );
              ...
              obj["received_fill"] =
                obj["received_fill"] +
                (lastStatusData.status == PKG_STATUS[1] ? 1 : 0);
              obj["loaded_craft"] =
                obj["loaded_craft"] +
                (lastStatusData.status == PKG_STATUS[2] ? 1 : 0);
              obj["received_nas"] =
                obj["received_nas"] +
                (lastStatusData.status == PKG_STATUS[4] ? 1 : 0);
              obj["ready_pd"] =
                obj["ready_pd"] +
                (lastStatusData.status == PKG_STATUS[5] ? 1 : 0);
              obj["delivered"] =
                obj["delivered"] +
                (lastStatusData.status == PKG_STATUS[6] ? 1 : 0);
            });
      ...
      if (filter === "all") {
        ...
        let result = await Promise.all(
          packages.map(async (pkg) => {
            let statuses =
              (await PackageStatus.find({ packageId: pkg._id })) || [];
            let packageStatus = statuses[statuses.length - 1];

            if (pkg.awbId.invoices.length == 0) {
              noDocs.push({
                _id: pkg.id,
                last_status: packageStatus.status,
                awb: pkg.awbId.id,
                customer_email: pkg.customerId.email,
              });
            }
        ...
        if (query.filter_date && query.filter_date !== "") {
          dbQuery = {
            createdAt: {
              $lte: moment(query.filter_date, "MM-DD-YYYY")
                .endOf("day")
                .toDate(),
              $gte: moment(query.filter_date, "MM-DD-YYYY")
                .startOf("day")
                .toDate(),
            },
          };
        ...
        const packages = await Package.find(dbQuery)
          .populate("awbId")
          .populate("customerId");
        ...
        let result = await Promise.all(
          packages.map(async (pkg) => {
            ...
            if (
              pkg.awbId.invoices.length == 0 &&
              query.filter_for === "noDocs" &&
              (query.package_status === packageStatus.status ||
                query.package_status === "all")
            ) {
              noDocs.push({
                _id: pkg.id,
                last_status: packageStatus.status,
            ...
            if (
              pkg.customerId.pmb == 9000 &&
              pkg.manifestId &&
              query.filter_for === "9to5" &&
              (query.package_status === packageStatus.status ||
                query.package_status === "all")
            ) {
              nineToPackages.push({
  ...
  async getAllPackagesWithLastStatus() {
    ...
    return await Promise.all(
      packages.map(async (pkg) => {
        let status = await this.services.packageService.getPackageLastStatus(
          pkg._id
        );
    ...
    return new Promise((resolve, reject) => {
      this.getAllPackages_updated().then((packages) => {
        Promise.all(
          ...
          stats.forEach((status, i) => {
            packages[i].lastStatusText = status.status;
            if (status.status == PKG_STATUS[4]) pkgs.push(packages[i]);
          });
          resolve(pkgs);
        });
      });
      ...
      return Promise.all(
        packageIds.map(async (packageId) => {
          ...
          if (br !== null) {
            ...
            if (pkg !== null) {
              response.push(
                await this.updatePackageStatus(pkg._id, 1, username)
              );
            } else {
              console.log("ssss", pkg);
              errors.push({
                success: false,
  ...
  addToFlight(packageIds, manifestId, compartmentId, userId) {
    return new Promise((resolve, reject) => {
      let packages =
        packageIds &&
        packageIds.length &&
        packageIds.split(",").filter(Boolean);
      ...
      Promise.all(
        ...
        console.log("errr", err, "resultss", results);
        resolve({
          success: true,
          message: strings.string_response_loaded,
          status: PKG_STATUS[2],
        });
      });
    });
  ...
  getCustomerPackages(customerId) {
    return new Promise((resolve, reject) => {
      Package.find({ customerId }).exec((error, packages) => {
        if (error) {
          resolve({ success: false });
        } else {
          resolve(packages);
  ...
  storePackageForPickup(trackingNo, bin) {
    ...
    return new Promise((resolve, reject) => {
      lredis
        .hmset(PACKAGE_ID + trackingNo, { status: 4, location: bin })
        .then((result) => {
  ...
  getGlobalSearchData(bodyData) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        return resolve({
          success: false,
          message: strings.string_global_search_error,
        });
      }

      if (selectedOption === "Package") {
        Package.find(
          { description: { $regex: "aWB package  post box", $options: "i" } },
          "id",
          (err, packages) => {
            if (err) {
              resolve([]);
        ...
        inputField = inputField.trim();
        Customer.find(
          { email: { $regex: "^" + inputField + "$", $options: "i" } },
          "_id",
          (err, customers) => {
            if (err) {
              resolve([]);
