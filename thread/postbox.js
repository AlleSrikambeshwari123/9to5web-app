...
var searchData = {pmb:{$ne:9000},createdAt : {"$gte":stdate, "$lte": endate}};
...
var d = new Date();
var time = d.getTime();
var filename = time+'_postboxetcpackage.csv'
createConnection()
  .then(() => {
    PackageHistory.aggregate([
      ...
      csvWriter.writeRecords(records)       // returns a promise
        .then(async() => {  
          ...
          await Mail.sendReportEmail(workerData.email,"Post Boxes Etc Packages Report", html);
          console.log(workerData.email);
          var detail = {
            reportType: 'POSTBOXETCPACKAGE',
            dateFrom:stdate,
            dateTo:endate,
            dateRange: daterange,
          ...
          console.log('...Done POSTBOXETCPACKAGE detail');
        });
    })
  })
